...
async def list_stocks(
    ...
    exclude_own: bool = Query(False, description="Exclude current tenant's stocks"),
...
async def search_stocks_html(
    ...
    """
    query = search.strip()
    # Strip flyroom:// prefix from QR code scans so search works like 1D barcodes
    if query.startswith("flyroom://"):
        query = query[len("flyroom://") :]
